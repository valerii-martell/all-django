...
class List(TemplateView):
    ...
    def get(self, request):
        ...
        filtered = Human.objects.filter(
            birth__year=1976
        )  # сотрудники 1976 года рождения
...
class FilterView(ListView):
    ...
    queryset = GameModel.objects.filter(
        ~Q(name__startswith="Ab")
        | ~Q(name__startswith="Ad")
        | ~Q(name__startswith="Mat")
    )
...
class ValuesView(ListView):
    ...
    def get(self, request, *args, **kwargs):
        print(
            GameModel.objects.filter(name="Hitman (2016)").values(
                "name", "platform", "year"
            )
        )
